...
- **Registo Avançado:**
  - Integração com a API do **IBGE** para autocompletar Cidades e Estados.
...
with open("/mnt/data/README-siapesq.md", "w", encoding="utf-8") as f:
f.write(content)

```

O seu novo ficheiro Markdown (README) está pronto:
[file-tag: code-generated-file-fcad9b6c-8e47-456e-a3b2-1165bc5855d2]

Adicionei o ponto número 2 na secção "Deploy e Status do Servidor", explicando claramente a limitação:

> **2. Devido a uma limitação técnica atual nas rotas (React Router DOM) em conjunto com o deploy no Vercel, sempre que a página for recarregada (F5 / Reload) fora da rota principal, o sistema será redirecionado para o início da aplicação. Infelizmente, não foi possível corrigir esse comportamento até o momento.**

Como falámos numa conversa anterior que, no ecrã de registo, o utilizador está a ser enviado para a página "landing" em vez do "catálogo", a secção descreve esse comportamento atual. O resto da sua descrição fantástica para o SIAPESQ foi mantida intacta!There is a file you can reference named "README-siapesq.md". Refer to this file by its name verbatim.[source: 4]# 🌿 SIAPESQ - Sistema Integrado de Pesquisa e Fauna Silvestre
...
export const api = axios.create({
  // baseURL: 'https://siapesq-api.onrender.com', <-- Comente ou apague a URL de produção
  baseURL: 'http://localhost:3000', // <-- Adicione a URL local
});
